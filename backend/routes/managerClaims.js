...
function updateDashboardStats(claimsData) {
  ...
  function setupFilters() {
    ...
    // Set default date values - today for "to" date field
  ...
  // Update the filterClaims function to handle date range
    ...
    claims.forEach(claim => {
      ...
      if (packageStatusFilter === "with-package" && claim.package_id) {
        ...
        if (claim.package_id) {
          ...
          if (claim.cost !== undefined && claim.cost !== null) packageInfo += `<br>Cost: ${claim.cost}`;
        }
